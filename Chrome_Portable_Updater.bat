...
::   更新日期: 2026-05-13
::   作者: AI助手 & ZhangSir
::   【注意】：保存此文件时，编码请务必选择 ANSI (或系统默认 GBK)
:: ============================================================================
...
:: 2. 提取 PowerShell (最简稳定引擎，无惧空格路径)
set "PS_SCRIPT=%temp%\chrome_updater_ps.ps1"
if exist "%PS_SCRIPT%" del "%PS_SCRIPT%"

powershell -NoProfile -Command "$c = Get-Content -LiteralPath '%~f0'; $i = [array]::IndexOf($c, '__POWERSHELL_BEGIN__'); if($i -ge 0){ $c[($i+1)..($c.Count-1)] | Out-File -Encoding UTF8 '%PS_SCRIPT%' }"

:: 3. 运行核心逻辑
if exist "%PS_SCRIPT%" (
    powershell -NoProfile -ExecutionPolicy Bypass -File "%PS_SCRIPT%"
) else (
    echo [Fatal Error] Script extraction failed. Please check file encoding - Must be ANSI.
    pause
)
...
function Show-Header {
    ...
    Write-Host "║        CHROME PORTABLE LAUNCHER & UPDATER (GitHub Edition)           ║" -ForegroundColor Cyan
    Write-Host "╟──────────────────────────────────────────────────────────────────────╢" -ForegroundColor Cyan
    Write-Host "║  Version: 26.05.14  |  Build: 2026-05-13  |  Source: Iplaysoft     ║" -ForegroundColor Cyan
    Write-Host "╚══════════════════════════════════════════════════════════════════════╝" -ForegroundColor Cyan
    Write-Host ""
}
